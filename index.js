...
async function run_loop(scraper_func, data_dir_path, is_test, sleep_time) {
  data_dir_path = expandHomeDir(data_dir_path)
  const date_str = moment(new Date()).local().format('YYYY-MM-DD_hh-mm')

  try {
    await run_once(scraper_func, is_test, date_str, data_dir_path)
  } catch(err) {
  ...
  setTimeout(async () => {
    await run_loop(scraper_func, data_dir_path, is_test, sleep_time)
  }, sleep_time || 1000 * 60 * 60 * 24)
}

async function run_once(scraper_func, is_test, date_str, data_dir_path) {
  ...
  await scraper_func(is_test, write_to_scraper_dir)
}
